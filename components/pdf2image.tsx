...
export default function Index(): JSX.Element {
  ...
  return (
    <div>
      {files && (
        <PdfViewer file={files[0]} width={500} pageNumber={pageNum} setTotalPageNum={setDocInfo} />
      )}
      ...
      <p>
        Page {pageNum} of {docInfo.numPages}
      </p>

      <button disabled={pageNum <= 1} onClick={() => setPageNum(pageNum - 1)}>
        Prev
      </button>
      <button disabled={docInfo.numPages <= pageNum} onClick={() => setPageNum(pageNum + 1)}>
        Next
      </button>
    </div>
  );
